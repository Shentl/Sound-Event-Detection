...
class Try2_4conv(nn.Module):
    def __init__(self, num_freq, class_num):
        super(Try2_4conv, self).__init__()
        ##############################
        # YOUR IMPLEMENTATION
        # Args:
        ...
        # [b_s, c, T/8, F/8]
        self.fc = nn.Linear(256*2, class_num)
        # self.fc2 = nn.Linear(128, class_num)
        self.rnn = nn.GRU(input_size=128, hidden_size=256, num_layers=1, batch_first=True, dropout=0, bidirectional=True)
        """
    ...
    def detection(self, x, debug=False):
        ...
        # BiGRU
        # out = torch.tanh(self.fc(out))  # [32, 501, 10] [b_s, T, class_num]
        out = self.fc(out)
