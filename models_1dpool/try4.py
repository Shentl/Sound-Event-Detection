...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1, kernel_size=3):
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=1, padding=(kernel_size-1)//2)
...
class Try4_4conv(nn.Module):
    def __init__(self, num_freq, class_num):
        super(Try4_4conv, self).__init__()
        ##############################
        # YOUR IMPLEMENTATION
        # Args:
        ...
        ##############################
        self.block1 = ConvBlock(in_channels=1, out_channels=16, kernel_size=3)
        self.block2 = ConvBlock(in_channels=16, out_channels=32, kernel_size=3)
        self.block3 = ConvBlock(in_channels=32, out_channels=64, kernel_size=3)
        self.block4 = ConvBlock(in_channels=64, out_channels=128, kernel_size=3)
        self.global_pool = nn.AdaptiveAvgPool2d((501, 1))
        # [b_s, c, T/8, F/8]
        self.fc = nn.Linear(128*2, class_num)
        self.rnn = nn.GRU(input_size=512, hidden_size=128, num_layers=1, batch_first=True, dropout=0, bidirectional=True)
        """
    ...
    def detection(self, x, debug=False):
        ...
        # out = out.permute(0, 2, 1, 3)  # stack at freq_dim [b_s, t, c, f/8]
        out = out.permute(0, 2, 3, 1)  # stack at time_dim [b_s, t, f/8, c]
        out = out.reshape(out.shape[0], out.shape[1], -1)  # [32, 501, 512] [b_s, t, cf/8]
        # print('out', out.shape)
        # [b_s, T, c]
